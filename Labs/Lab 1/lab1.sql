...
CONSTRAINT  total_price_check CHECK (total_price>0 AND total_price<100000)
);
...
CONSTRAINT  customer_check CHECK (subscription_type='p' OR subscription_type='s')
);
...
SELECT h.hotel_name, h.star_rating, p.customer_id
FROM hotel h 
INNER JOIN hotel_package hp ON h.hotel_id = hp.hotel_id
INNER JOIN holiday hy ON hp.package_id=hy.package_id
INNER JOIN purchase p ON hy.package_id=p.package_id
WHERE h.star_rating = 3 AND p.customer_id = 1;
